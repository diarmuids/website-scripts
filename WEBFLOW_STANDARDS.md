...
| Styles set on structural pseudo-states (`nth-child(odd)`, `nth-child(even)`, `first-child`, `last-child`) through the style tool render in the browser, but the Designer only exposes those states on Collection List items. On ordinary elements the values show on the canvas yet can't be selected or edited. | Don't use structural pseudo-states for per-item differences outside Collection Lists. Give the affected item a visible `is-` combo class (for example `about-founders_item is-divider`) so the value is editable in the Style panel. |
