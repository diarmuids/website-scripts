...
| Non-native CSS (pseudo-elements, `:has()`, `:not()`, `:focus-within`, `@supports`, `clamp()`, `mask`, `line-clamp`, scrollbar styling, keyframes and so on) can't be seen or edited in the Style panel. | Use it only when nothing native works, or when the user explicitly asks for it. Put it in the site's `local_styles` Code Embed only, comment every rule (what, where, why it isn't native), and flag it to the user in the reply as **Please note — non-native CSS added**. |
