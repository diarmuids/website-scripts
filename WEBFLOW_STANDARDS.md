...
| Styles set on structural pseudo-states (`nth-child(odd)`, `nth-child(even)`, `first-child`, `last-child`) through the style tool render in the browser, but the Designer only exposes those states on Collection List items. On ordinary elements the values show on the canvas yet can't be selected or edited. | **Only style states the Designer's States menu offers for that element.** Everywhere: None, Hover, Active (pressed), Focused, Focused (keyboard), plus Current (links and nav links), Current Locale and Visited (links). On Collection List items, also First, Last, Odd and Even. Never use any other pseudo-class or pseudo-element through the style tool: `nth-child(n)` with any other formula, `first-child`/`last-child`/`nth-child` outside Collection Lists, `::before`, `::after`, `:focus-within`, `:not()` and so on. For per-item differences, give the item a visible `is-` combo class (for example `about-founders_item is-divider`) so every value is editable in the Style panel. |
| Non-native CSS (pseudo-elements, `:has()`, `:not()`, `:focus-within`, `@supports`, `clamp()`, `mask`, `line-clamp`, scrollbar styling, keyframes and so on) can't be seen or edited in the Style panel. | Avoid it wherever possible. `calc()`, `clamp()`, `min()` and `max()` typed into native value fields are fine. Properties with no native control may go in a class's Custom Properties. Ask the user before using `::before` / `::after`. Anything else goes in the site's `local_styles` Code Embed only, with every rule commented (what, where, why it isn't native). Whenever anything non-native is added, the reply must include the heading `## ⚠️ PLEASE NOTE: NON-NATIVE STYLES ADDED`, listing each rule (see AGENTS.md). |
